...
class AgendouAgentService
{
    ...
    public function processar(
        ...
        for ($i = 0; $i < 6; $i++) {
            try {
                $response = $this->ai->processar($tenant, [
                    'max_tokens' => 1024,
                    'system' => $systemBlocks,
                    'tools' => $tools,
                    'messages' => $messages,
                ]);
            } catch (AiProviderException|AiLimitExceededException $e) {
                Log::channel('jobs')->error('AI_ORCHESTRATOR_ERROR', [
                    'tenant_id' => $tenant->id,
                    'exception' => $e::class,
