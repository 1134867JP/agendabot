...
export default function TenantDashboard({
    ...
    const possuiRelatorios = tenant.plano === 'pro' || tenant.plano === 'business' || Boolean(subscription?.isento_cobranca);
    const hrefRelatorios = possuiRelatorios ? route('tenant.analytics') : route('tenant.renovar');
                ...
                {modoAgendamento && (
                    <section className="rounded-xl p-4 sm:p-5" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <Link href={hrefRelatorios} className="btn-secondary min-h-11">{possuiRelatorios ? 'Ver desempenho' : 'Conhecer relatórios'}</Link>
                        </div>
                        {ultima_cobranca_bot && (
