...
export default function Home() {
    ...
    return (
        <LandingLayout currentPage="home">
            ...
            {/* ── Preços ────────────────────────────────────────────────────── */}
            <section id="precos" className="px-6 py-24" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="mx-auto max-w-4xl">
                    <div className="mb-14 text-center">
                        <p className="mb-2 text-xs font-semibold uppercase tracking-[0.15em]" style={{ color: JADE }}>Preços</p>
                        <h2
                            className="text-4xl"
                            style={{ fontFamily: 'Instrument Serif, Georgia, serif', color: 'rgba(232,230,225,0.9)' }}
                        >
                        ...
                        <p className="mt-3 text-sm" style={{ color: 'rgba(232,230,225,0.45)' }}>
                    ...
                    <div className="grid gap-5 md:grid-cols-3">
                        {PLANOS.map(plano => (
                            <div
                                ...
                                style={{
                                    background: plano.destaque ? `${INDIGO}12` : 'rgba(255,255,255,0.03)',
                                    border: `1.5px solid ${plano.destaque ? `${INDIGO}55` : 'rgba(255,255,255,0.08)'}`,
                                }}
                            >
                                {plano.destaque && (
                                    <span
                                ...
                                <div className="mt-2 mb-1">
                                    <span className="text-3xl font-bold" style={{ color: 'rgba(232,230,225,0.95)' }}>
                                        R${plano.valor.toFixed(2).replace('.', ',')}
                                    </span>
                                    <span className="text-sm" style={{ color: 'rgba(232,230,225,0.35)' }}>/mês</span>
                                </div>
                                <p className="mb-5 text-xs" style={{ color: 'rgba(232,230,225,0.4)' }}>{plano.descricao}</p>
                                <ul className="mb-6 flex-1 space-y-2.5">
                                    {plano.features.map(f => (
                                        <li key={f} className="flex items-start gap-2 text-sm" style={{ color: 'rgba(232,230,225,0.7)' }}>
                                            <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0" style={{ color: JADE }}>
                                                <polyline points="20 6 9 17 4 12"/>
                                            </svg>
                                            {f}
                                        </li>
                                ...
                                <Link
                                    href={route('onboarding.step1')}
                                    className="block rounded-xl py-2.5 text-center text-sm font-semibold transition-all hover:brightness-110"
                                    style={plano.destaque
                                        ? { background: INDIGO, color: 'white', boxShadow: `0 8px 24px -6px ${INDIGO}50` }
                                        : { background: 'rgba(255,255,255,0.06)', color: 'rgba(232,230,225,0.65)', border: '1px solid rgba(255,255,255,0.1)' }
                                    }
                                >
                                    Começar grátis
                                </Link>
