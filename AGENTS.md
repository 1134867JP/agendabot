# Prompt Codex — SaaS de Agendamento via WhatsApp
...
- O **bot** usa Codex API (Haiku 4.5) para entender linguagem natural e conduzir o agendamento
...
- **IA**: Codex API — `Codex-haiku-4-5` (Anthropic)
...
CLAUDE_MODEL=Codex-haiku-4-5-20251001
```
...
Schema::create('conversas', function (Blueprint $table) {
    ...
    // Array de {role: user|assistant, content: string} para o Codex
...
class ClaudeService
{
    ...
    {
        $this->apiKey = config('services.Codex.key');
        $this->model  = config('services.Codex.model'); // Codex-haiku-4-5-20251001
    }
    ...
    public function processarMensagem(
        ...
        // Codex deve responder em JSON estruturado
...
class BotService
{
    ...
    public function processarWebhook(Tenant $tenant, string $telefone, string $mensagem): void
    {
        ...
        // 3. Processar com Codex
        $resultado = $this->Codex->processarMensagem($tenant, $conversa, $mensagem);
...
'Codex' => [
    ...
    'model' => env('CLAUDE_MODEL', 'Codex-haiku-4-5-20251001'),
],
...
- [ ] Codex API integrada com system prompt dinâmico por tenant
...
- **Custo Codex API estimado**: com Haiku 4.5 a $1/MTok input e $5/MTok output, uma conversa completa de ~800 tokens custa ~$0,003. Para 1.000 agendamentos/mês por tenant: ~$3/mês.
